...
from topic_modeling_offensive_utils import OFFENSIVE_PATH
from topic_modeling_offensive_utils import SAVE_MODEL_PATH, SAVE_DOCUMENT_INFO_PATH, CURRENT_YEAR, CURRENT_EXPERIMENT_CLASS 
...
def topic_modeling(documents, save_as):
    ...
    topic_model = BERTopic(language='english', diversity=0.2, umap_model=umap_model)
...
def main():
    ...
    if CURRENT_EXPERIMENT_CLASS == 'offensive':
        file_path = OFFENSIVE_PATH
        file_name_tokens = current_file_name.split('_')
        new_name = 'topic_modeling_offensive_compliant_seed42'
