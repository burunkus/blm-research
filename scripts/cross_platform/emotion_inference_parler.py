    ...
    def __init__(self,
        ...
        with open(data_path) as csv_file_handle:
            ...
            for i, line in enumerate(csv_reader):
                self.comment_ids.append(line[0].strip())
                self.author_ids.append(line[1].strip())
                self.dates.append(line[2].strip())
                self.comments.append(line[3].strip())

        tokenized_tweet = tokenizer(self.comments,
                                    padding=padding,
...
def classify(dataloader, tokenizer, model, save_as):
    ...
    with open(save_as, mode='w') as csv_file_handle:
        ...
        for step, batch in enumerate(dataloader):
            ...
            batch_input_ids, batch_input_masks, batch_token_type_ids, batch_comment_ids,\
            batch_author_ids, batch_dates, batch_comments = batch
...
if __name__ == "__main__":
    ...
    data = DATA_PATH + "parler_blm_preprocessed_2020.csv"
    save_as = DATA_PATH + "parler_blm_preprocessed_2020_emotion.csv"
